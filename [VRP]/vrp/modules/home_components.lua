    ...
    if user_id ~= nil and user_id == owner_id then
      ...
      if data.cloakroom_idle ~= nil then
        -- vRPclient.notify(player,{lang.common.wearing_uniform()})
		   TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.common.wearing_uniform()}, type = "warning", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
          ...
          if amount > 0 then
            if vRP.tryPayment(user_id,amount) then
              -- vRPclient.notify(player,{lang.home.gametable.bet.started()})
				TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.gametable.bet.started()}, type = "info", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              ...
              local close_bet = function()
                if bet_opened then
                  ...
                  if wuser_id ~= nil then
					...
					TriggerClientEvent("pNotify:SendNotification", wplayer,{text = {lang.money.received({bet_total})}, type = "success", queue = "global", timeout = 3000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                      ...
                      if ok and bet_opened then
                        if vRP.tryPayment(nuser_id,amount) then -- register player bet
                          ...
                          -- vRPclient.notify(nplayer,{lang.money.paid({amount})})
							TriggerClientEvent("pNotify:SendNotification", nplayer,{text = {lang.money.paid({amount})}, type = "success", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        else
                          -- vRPclient.notify(nplayer,{lang.money.not_enough()})
							TriggerClientEvent("pNotify:SendNotification", nplayer,{text = {lang.money.not_enough()}, type = "error", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        end
                      end
