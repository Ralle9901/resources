        ...
        if huser_id ~= nil then
          if huser_id == user_id then -- identify owner (direct home access)
            vRP.accessHome(user_id, home_name, number, function(ok)
			  ...
			  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.intercom.not_available()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              end
            end)
              ...
              vRP.prompt(player,lang.home.intercom.prompt_who(),"",function(player,who)
                ...
                vRP.request(hplayer, lang.home.intercom.request({who}), 30, function(hplayer,ok)
                  if ok then
					...
					TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.intercom.refused()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                  end
                end)
              end)
            else
        ...
        vRP.findFreeNumber(home_name, home.max, function(number)
          if number ~= nil then
            if vRP.tryPayment(user_id, home.buy_price) then
			  ...
			  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.buy.bought()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            else
              -- vRPclient.notify(player,{lang.money.not_enough()})
			  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.not_enough()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          else
            -- vRPclient.notify(player,{lang.home.buy.full()})
			TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.buy.full()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
          end
        end)
      -- else
  ...
  menu[lang.home.sell.title()] = {function(player,choice)
    vRP.getUserAddress(user_id, function(address)
      if address ~= nil and address.home == home_name then -- check if already have a home
		...
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.sell.sold()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      else
        -- vRPclient.notify(player,{lang.home.sell.no_home()})
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.home.sell.no_home()}, type = "info", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
    end)
