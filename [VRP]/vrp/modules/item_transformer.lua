    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units >= 1 and recipe then -- check units
        ...
        if new_weight > vRP.getInventoryMaxWeight(user_id) then
		  ...
		  TriggerClientEvent("pNotify:SendNotification", tonumber(k),{text = {lang.inventory.full()}, type = "warning", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
...
local function ch_informer_buy(player,choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if vRP.tryPayment(user_id, price) then
	  ...
	  vRPclient.setNamedBlip(player,{tr.itemtr.name,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z,432,46,tr.itemtr.name}) -- add temporary map blip
      -- vRPclient.notify(player, {lang.money.paid({price})})
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.paid({price})}, type = "success", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      -- vRPclient.notify(player, {lang.itemtr.informer.bought()})
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.itemtr.informer.bought()}, type = "success", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    else
