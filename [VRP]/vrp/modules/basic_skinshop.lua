...
function vRP.openSkinshop(source,parts)
  ...
  if user_id ~= nil then
    ...
    if data.cloakroom_idle ~= nil then
	  ...
	  TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.common.wearing_uniform()}, type = "warning", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
        ...
        vRPclient.getCustomization(source,{},function(custom)
          ...
          if vRP.tryPayment(user_id,price) then
            if price > 0 then
			  ...
			  TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.money.paid({price})}, type = "success", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          else
