    ...
    if v > 0 then
      ...
      if user_id ~= nil then
        if vRP.tryDeposit(user_id,v) then
		  ...
		  TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.atm.deposit.deposited({v})}, type = "success", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        else
          -- vRPclient.notify(source,{lang.money.not_enough()})
		  TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.money.not_enough()}, type = "error", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      end
    else
      -- vRPclient.notify(source,{lang.common.invalid_value()})
	  TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.common.invalid_value()}, type = "error", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
  end)
end
  ...
  vRP.prompt(source,lang.atm.withdraw.prompt(),"",function(player,v)
    ...
    if v > 0 then
      ...
      if user_id ~= nil then
		...
		TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.atm.withdraw.withdrawn({v})}, type = "success", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        else
          -- vRPclient.notify(source,{lang.atm.withdraw.not_enough()})
		TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.atm.withdraw.not_enough()}, type = "error", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      end
    else
      -- vRPclient.notify(source,{lang.common.invalid_value()})
