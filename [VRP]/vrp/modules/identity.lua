                ...
                if vRP.tryPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
                      ...
                      vRPclient.setRegistrationNumber(player,{registration})
                      -- vRPclient.notify(player,{lang.money.paid({cfg.new_identity_cost})})
					  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.paid({cfg.new_identity_cost})}, type = "success", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                    end)
                  end)
                else
                  -- vRPclient.notify(player,{lang.money.not_enough()})
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
  ...
  vRP.getUserIdentity(user_id, function(identity)
    if identity then
      vRPclient.setRegistrationNumber(source,{identity.registration or "AAAAAA"})
    end
  end)
      ...
      if identity then
		...
		-- get driver license status
          ...
          vRP.getDriverLicense(user_id, function(license)
            ...
            if driverlicense == "Required" then
              driverlicense = "Nej"
            else
              driverlicense = "Ja"
            end

          local content = lang.cityhall.menu.info({htmlEntities.encode(identity.name),htmlEntities.encode(identity.firstname),identity.age,identity.registration,identity.phone,home,number,driverlicense})
          local choices = {}
          choices[lang.cityhall.menu.title()] = {function()end, content}
