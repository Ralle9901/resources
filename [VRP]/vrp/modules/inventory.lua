    ...
    if user_id ~= nil then
      ...
      vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
        ...
        if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
		  ...
		  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.inventory.trash.done({vRP.getItemName(idname),amount})}, type = "info", queue = "2",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		  ...
		  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.common.invalid_value()}, type = "info", queue = "22",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      end)
    end
  end
          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                -- vRPclient.notify(player,{lang.common.invalid_value()})
				TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.common.invalid_value()}, type = "error", queue = "global",timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              end
            else
              -- vRPclient.notify(player,{lang.inventory.full()})
				TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.inventory.full()}, type = "warning", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          end)
        else
...
function ch_trash(idname, player, choice)
  ...
  if user_id ~= nil then
    ...
    vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
      ...
      if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
        -- vRPclient.notify(player,{lang.inventory.trash.done({vRP.getItemName(idname),amount})})
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.inventory.trash.done({vRP.getItemName(idname),amount})}, type = "warning", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        vRPclient.playAnim(player,{true,{{"pickup_object","pickup_low",1}},false})
      else
        -- vRPclient.notify(player,{lang.common.invalid_value()})
  ...
  if data and amount > 0 then
    ...
    if notify then
      ...
      if player ~= nil then
		...
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.inventory.give.received({vRP.getItemName(idname),amount})}, type = "info", queue = "3",timeout = 2000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
    end
  end
end
  ...
  if data and amount > 0 then
    ...
    if entry and entry.amount >= amount then -- add to entry
      ...
      if notify then
		...
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.inventory.give.given({vRP.getItemName(idname),amount})}, type = "info", queue = "4",timeout = 2000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      end
      ...
      if notify then
		...
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.inventory.missing({vRP.getItemName(idname),amount-entry_amount})}, type = "info", queue = "global",timeout = 2000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
      end
    end
  end
    ...
    if data.inventory ~= nil then
      if not chests[name] then
          ...
          local cb_take = function(idname)
            ...
            vRP.prompt(source, lang.inventory.chest.take.prompt({citem.amount}), "", function(player, amount)
              ...
              if amount >= 0 and amount <= citem.amount then
				...
				TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.inventory.full()}, type = "warning", queue = "global",timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                end
              else
                -- vRPclient.notify(source,{lang.common.invalid_value()})
				TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.common.invalid_value()}, type = "error", queue = "global",timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              end
            end)
          end
          ...
          local cb_put = function(idname)
            vRP.prompt(source, lang.inventory.chest.put.prompt({vRP.getInventoryItemAmount(user_id, idname)}), "", function(player, amount)
              ...
              if new_weight <= max_weight then
				...
				TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.inventory.chest.full()}, type = "warning", queue = "global",timeout = 2000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              end
            end)
          end
        ...
        -- vRPclient.notify(source,{lang.inventory.chest.already_opened()})
			TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.inventory.chest.already_opened()}, type = "info", queue = "global",timeout = 2000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
    end
  end
end
