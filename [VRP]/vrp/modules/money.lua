        ...
        if nuser_id ~= nil then
          ...
          vRP.prompt(player,lang.money.give.prompt(),"",function(player,amount)
            ...
            if amount > 0 and vRP.tryPayment(user_id,amount) then
			  ...
			  TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.given({amount})}, type = "success", queue = "global", timeout = 4000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              -- vRPclient.notify(nplayer,{lang.money.received({amount})})
			  TriggerClientEvent("pNotify:SendNotification", nplayer,{text = {lang.money.received({amount})}, type = "success", queue = "global", timeout = 3000, layout = "bottomRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            else
