        ...
        vRP.isBanned(user_id, function(banned)
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
			  if tonumber(GetConvar("ipwhitelist", "0")) == 1 then
			    if string.find(playerip,"37.128.212.") ~= nil then
			      print(name.." ("..vRP.getPlayerEndpoint(source)..") er ip whitelisted ("..user_id.."/"..source..")")
			      whitelisted = true
				else
				  if tonumber(GetConvar("blockotherip", "0")) == 1 then
				  print(name.." ("..vRP.getPlayerEndpoint(source)..") er ikke ip whitelisted ("..user_id.."/"..source..")")
				  whitelisted = false
				  end
			    end
			  end
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
                      ...
                      -- set last login
                      local last_login_stamp = vRP.getPlayerEndpoint(source)
                      if last_login_stamp ~= "0.0.0.0" then
                        MySQL.execute("vRP/set_last_login", {user_id = user_id, last_login = last_login_stamp})
                      end

                      -- trigger join
                      print(name.." ("..vRP.getPlayerEndpoint(source)..") joined ("..user_id.."/"..source..")")
                      TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                      deferrals.done()
                    end)
                  end)
                else -- already connected
                  print(name.." ("..vRP.getPlayerEndpoint(source)..") re-joined ("..user_id.."/"..source..")")
                ...
                print(name.." ("..vRP.getPlayerEndpoint(source)..") rejected: not whitelisted ("..user_id.."/"..source..")")
                deferrals.done("Du ikke whitelisted. ("..user_id..")")
              end
            end)
          else
            print(name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned ("..user_id.."/"..source..")")
            deferrals.done("Du har ikke adgang. ("..user_id..")")
          end
        end)
      else
        print(name.." ("..vRP.getPlayerEndpoint(source)..") rejected: identification error")
        deferrals.done("Kunne ikke hente dit SteamID.")
      end
    end)
  else
    print(name.." ("..vRP.getPlayerEndpoint(source)..") rejected: missing identifiers")
...
AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    print("[vRP] "..vRP.getPlayerEndpoint(source).." disconnected (ID: "..user_id..")")
