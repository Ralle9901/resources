...
function renderBoxes()
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		...
		if isRadarExtended then
			local percent = (essence/0.122)*100

			DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.35,"", 255, 255, 255, 255, 0, 1)
		elseif(hud_form == 1) then
			local percent = (essence/0.122)*100

			DrawAdvancedText(text_x, text_y, 0.001, 0.0010, 0.35,round(percent,1).."%", 255, 255, 255, 255, 0, 1)
		else
			local percent = (essence/0.122)*100
