...
AddEventHandler('es_bank:toofarlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', 'NYHEDER', {255, 0, 0}, "Røveriet blev aflyst, du vil ikke modtage noget.")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_bank:playerdiedlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', 'NYHEDER', {255, 0, 0}, "Røveriet blev aflyst, du døde!.")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_bank:robberycomplete', function(reward)
	robbing = false
	TriggerEvent('chatMessage', 'NYHEDER', {255, 0, 0}, "Røver gjort, modtog du:^2" .. reward)
	bank = ""
	secondsRemaining = 0
	incircle = false
end)
...
Citizen.CreateThread(function()
	for k,v in pairs(banks)do
		...
		AddTextComponentString("Bankrøveri")
...
Citizen.CreateThread(function()
	while true do
				...
				if not robbing then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2)then
						if (incircle == false) then
							bank_DisplayHelpText("Tryk ~INPUT_CONTEXT~ For at røv ~b~" .. v.nameofbank .. "~w~ pas på, politiet bliver advaret!")
						end
						incircle = true
		...
		if robbing then
			...
			bank_drawTxt(0.66, 1.44, 1.0,1.0,0.4, "Bank: ~r~" .. secondsRemaining .. "~w~ sekunder tilbage", 255, 255, 255, 255)
