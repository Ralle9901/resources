  ...
  local tattoshop_choice = function(player,choice)
    ...
    if tattoo then
	  ...
	  if tattoo == "CLEAR" then-- get player weapons to not rebuy the body
		...
		TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.money.paid({price})}, type = "success", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
        else
		...
		vRP.getUData({user_id,"vRP:tattoos",function(value)
		  ...
		  if not owned then
            ...
            -- vRPclient.notify(player,{"Du har allerede den tattoo"})
			TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har allerede denne tattoo", type = "error", queue = "global", timeout = 4000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
		  end
		end})
	  end
    end
