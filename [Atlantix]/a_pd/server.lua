...
AddEventHandler('door:status', function(id, status)
  ...
  if vRP.hasPermission({user_id, "#"..doorList[id].key..".>0"}) or vRP.hasPermission({user_id,doorList[id].permission}) then
    ...
    if status then
      vRPclient.pnotify(player, {"Dør ~r~låst ~y~med ~b~nøgle"..doorList[id].name})
    else
      vRPclient.pnotify(player, {"Dør ~g~ulåst ~y~med ~b~nøgle"..doorList[id].name})
    end
  else
    vRPclient.pnotify(player, {"~r~Mangler ~b~Key "..doorList[id].name})
  end 
end)
