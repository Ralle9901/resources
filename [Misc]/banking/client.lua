...
local atms = {
  ...
  {name="Hæveautomat", id=277, x=149.4551, y=-1038.95, z=29.366},
    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {255, 255, 255}, "Tryk ^5E^0 for at tilgå banken");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Dette kan du ikke gøre fra et køretøj");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Hæveautomaten kan ikke bruges i bilen");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
      TriggerEvent('chatMessage', "BANK", {0, 200, 0}, "^0Du indsatte ^5"..amount.." DKK");
    end
  else
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Du kan ikke bruge banken i bilen!");
    else
      TriggerServerEvent("bank:withdraw", tonumber(amount))
      TriggerEvent('chatMessage', "BANK", {0, 200, 0}, "^0Du hævede ^5"..amount.. " DKK");
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Dette er ikke en bank eller hæveautomat!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Spilleren er ikke online!");
    end
  else
