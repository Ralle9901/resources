...
AddEventHandler("chatMessage", function(s, n, m)
  ...
  if command[1] == "/balance" then
    TriggerClientEvent("chatMessage", s, "BANK", {0, 200, 0}, "Du har: ^3" .. bankbalance)
    TriggerClientEvent("banking:updateBalance", source, bankbalance)
    CancelEvent()
  end
  ...
  if command[1] == "/overfør" then
    if(command[2] ~= nil and tonumber(command[3]) > 0) then
      ...
      TriggerClientEvent('chatMessage', s, "BANK", {0, 200, 0}, "^1Brug /overfør [ID] [Beløb]^0")
    end
    CancelEvent()
  end
end)
  ...
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      ...
      if(rounded <= wallet) then
        ...
        vRPclient.notify(user_id,{"~r~Du har ikke nok penge!"})
      end
    end
end)
  ...
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      ...
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        vRPclient.notify(user_id, {"~r~Du har ikke nok penge på kontoen!"})
      end
    end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if tonumber(fPlayer) == tonumber(toPlayer) then
    ...
    local rounded = round(tonumber(amount), 0)
    if(string.len(rounded) >= 9) then
      TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Beløb for højt!")
      CancelEvent()
    else
      local bankbalance = vRP.getBankMoney({user_id})
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Overførte: ^1"..math.floor(rounded).. " DKK")
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Ny balance: ^4" .. math.floor((bankbalance - rounded)).. " DKK")
        ...
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Modtog: ^2"..math.floor(rounded).. " DKK")
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Ny balance: ^4" .. math.floor((bankbalance2 + rounded)).. " DKK")
        CancelEvent()
      else
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Du har ikke nok penge på kontoen!")
      end
    end    
  end
end)
